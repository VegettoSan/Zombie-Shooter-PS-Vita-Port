...
1. **FalsoNDK**: su `fndk_log()` débil imprimía `ALOGD/ALOGW/ALOGE` mediante `sceClibPrintf()` también en Release.
...
4. **so_util**: `SO_UTIL_VERBOSE=1` estaba definido globalmente, no sólo en Debug.
...
- `source/utils/logger.h`: Release conserva sólo agregados `[PERF]` y errores/fatales del logger del port.
- `source/reimpl/fndk_log.c`: override fuerte de `fndk_log()`.
  - Debug: conserva stream completo de FalsoNDK.
  - Release: silencia Debug/Warn/Error de FalsoNDK; conserva Fatal.
- `CMakeLists.txt`:
...
Commits relevantes de esta preparación:
...
Usar `Zombie-Shooter-Vita-Release.vpk` y registrar:
...
No eliminar estos shims sin volver a comprobar que la imagen nightly ya resolvió ambos problemas.
