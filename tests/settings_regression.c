...
int main(void) {
    settings_load();
    assert(setting_software_width==0);
    FILE *f=fopen(DATA_PATH "config.txt","w");assert(f);
    fputs("vita_shooter 1\nsetting_sampleSetting 7\n",f);fclose(f);
    settings_load();assert(setting_software_width==0 && setting_vita_shooter==1);
    ...
    puts("Settings regression PASS: safe default, 0/960/864 modes, invalid fallback and input preservation");
}
